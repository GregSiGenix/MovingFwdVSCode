...
#define USBH_CALLBACK_USE   /*lint -e{818}  N:104 */
#define USBH_API_USE        /*lint -e{818}  N:104 */
...
#ifdef USBH_TIME
  //
  // RTOS specific timestamp handling.
  // Used with a RTOS that is not able to provide a 32-bit timestamp in milliseconds.
  // To activate this:
  // (1) USBH_TIME must be defined (as a macro) in USBH_Conf.h to the type of the OS timestamp.
  // (2) The following functions must exist in the OS layer:
  //
  I32       USBH_TimeDiff(USBH_TIME TimeA, USBH_TIME TimeB);
  USBH_TIME USBH_INLINE USBH_TimeAdd(USBH_TIME a, U32 Offset);
#else
  //
  // Handle standard timestamps with 32-bit in milliseconds.
  //
  #if defined(__GNUC__) || defined(__clang__) || defined(__SEGGER_CC__) || defined(USBH_INLINE)
    //
    // Use inline functions for favorite compilers
    //
    #ifndef USBH_INLINE
      #define USBH_INLINE inline __attribute__((always_inline))
    #endif
...
*    If TimeA is before TimeB the result is negative.
*    If TimeA is after TimeB the result is positive.
*    Wrap around of the timestamp must be considered.
*/
    static USBH_INLINE I32 USBH_TimeDiff(USBH_TIME TimeA, USBH_TIME TimeB) {
      USBH_TIME TimeX = TimeA - TimeB;
      return (I32)TimeX;
    }
...
*
*  Function description
*    Add an offset in milliseconds to a timestamp.
*    Wrap around of the timestamp must be considered.
*/
    static USBH_INLINE USBH_TIME USBH_TimeAdd(USBH_TIME TimeBase, U32 Offset) {
      return TimeBase + (USBH_TIME)Offset;
    }
  #else
    //
    // Using macros for other compilers
    //
    #define USBH_TimeDiff(TimeA, TimeB)    ((I32)((TimeA) - (TimeB)))
    #define USBH_TimeAdd(TimeBase, Offset) ((TimeBase) + (USBH_TIME)(Offset))
  #endif
#endif
...
// Common macros
//
#define USBH_TIME_CALC_EXPIRATION(Timeout)   USBH_TimeAdd(USBH_OS_GetTime32(), Timeout)
//
// USBH_TIME_ISEXPIRED(T) is true, if current time is >= T
//
#define USBH_TIME_IS_EXPIRED(T)             (USBH_TimeDiff(USBH_OS_GetTime32(), T) >= 0)

/*********************************************************************
*
*       Definitons for mutex use with functions USBH_OS_Lock/Unlock().
*
**********************************************************************
*/
//
...
#define USBH_MUTEX_AUDIO     1      // AUDIO class usage
#define USBH_MUTEX_VIDEO     1      // VIDEO class usage
#define USBH_MUTEX_DRIVER    2      // Driver mutex
#define USBH_MUTEX_MSD       3      // MSD class usage
#define USBH_MUTEX_NET       3      // NET class usage
...
#define USB_CS_INTERFACE_DESCRIPTOR_TYPE              0x24u

// Defines for Standard Configuration Descriptor bmAttributes
...
#define USB_DEVICE_CLASS_VIDEO                        0x0Eu
#define USB_DEVICE_CLASS_WIRELESS                     0xE0u
#define USB_DEVICE_CLASS_VENDOR_SPECIFIC              0xFFu

// VIDEO protocol and subclass definitions
#define VIDEO_DEVICE_CONTROL_INTERFACE_SUBCLASS       0x01u
#define VIDEO_DEVICE_STREAM_INTERFACE_SUBCLASS        0x02u
...
#define USBH_HUB_PROTOCOL_MULTI_TT                    0x02u

// MIDI protocol and subclass definitions
#define USBH_AUDIO_CONTROL_SUBCLASS                   0x03u
...
// Hub class requests
#define USB_REQ_CLEAR_TT_BUFFER                       0x08u
...
#define USB_DESC_LENGTH_INDEX                         0u
#define USB_DESC_TYPE_INDEX                           1u

// Interface Descriptor indexes
#define USB_DESC_INTERFACE_NUMBER_INDEX               2u
#define USB_DESC_ALTERNATE_SETTING_INDEX              3u
...
//
#define USB_IA_DESCRIPTOR_LENGTH                      8
#define USB_IAD_FIRST_IF_OFS                          2
#define USB_IAD_IF_COUNT_OFS                          3
#define USB_IAD_FUNC_CLASS_OFS                        4
#define USB_IAD_FUNC_SUBCLASS_OFS                     5
#define USB_IAD_FUNC_PROT_OFS                         6
#define USB_IAD_STRING_INDEX_OFS                      7
...
#define PORT_STATUS_SUPER_SPEED                       0x00000600UL
#define PORT_STATUS_SPEED_IDX(x)                      (((x) >> 9) & 3u)
#define PORT_STATUS_30                                0x00000800UL
...
// Driver capabilities
typedef struct {
  USBH_SPEED   MaxSpeed;                     // Maximum supported speed by this controller.
  I8           NeedConfigureEPs;             // USBH_FUNCTION_CONFIGURE_EPS must be executed before EPs can be used
} USBH_DRIVER_CAPS;

// IO control functions
#define USBH_IOCTL_FUNC_GET_MAX_TRANSFER_SIZE      1u
#define USBH_IOCTL_FUNC_CONF_MAX_XFER_BUFF_SIZE    2u
#define USBH_IOCTL_FUNC_CONF_POWER_PIN_ON_LEVEL    3u
#define USBH_IOCTL_FUNC_GET_CAPABILITIES           4u
...
//
//lint -strong(AXJ, USBH_EP0_PHASE)
typedef enum {
  ES_IDLE   = 0,
  ES_SETUP,
  ES_DATA,
...
**********************************************************************
*/
//lint -strong(AXJ, USBH_SUBSTATE_STATE)
typedef enum {
  ...
  USBH_SUBSTATE_COMPLETE                                  // Urb completed
} USBH_SUBSTATE_STATE;
...
//
//lint -strong(AXJ, USBH_HUB_PORTRESET_STATE)
typedef enum {
...
//lint -strong(AXJ, USBH_HUB_ENUM_STATE)
...
//  USBH_HUB_ENUM_GET_STATUS,            // Get the device status
  ...
  USBH_HUB_ENUM_SET_ALTERNATE,         // Set alternate setting
  USBH_HUB_ENUM_DONE
} USBH_HUB_ENUM_STATE;
...
#define USBH_PORT_DO_SUSPEND          (1u << 6)
#define USBH_PORT_DO_RESUME           (1u << 7)

struct _USBH_HUB_PORT {
#if USBH_DEBUG > 1
  U32                  Magic;
#endif
  ROOT_HUB           * pRootHub;           // Set if root hub port
  USBH_HUB           * pExtHub;            // Set if external hub
  U32                  PortStatus;         // A copy of the port status returned from the HUB
  USBH_TIME            DelayUntil;         // Delay next port action until this system time is reached.
  U8                   ToDo;               // Bit mask of USBH_PORT_DO_... macros
  USBH_SPEED           PortSpeed;          // The current speed of the device
  I8                   DeviceEnumActive;   // Device enumeration in progress on this port.
...
//lint -strong(AXJ, USBH_HUB_ACTION)
typedef enum {
  USBH_HUB_ACT_IDLE,
...
struct _USBH_HUB { // USB HUB object
  ...
  U16                   PowerGoodTime;               // Power on to power good time in ms
  U16                   Characteristics;
  unsigned              PortCount;                   // Number of ports
  USBH_HUB_PORT       * pPortList;                   // Array of ports
  U8                    MultiTT;                     // == 1, if hub runs in multi TT mode.
  ...
  U16                   InterruptTransferBufferSize;

  U8                    InterfaceNo;                 // Hub interface of the device
  U8                    MultiTTAltSetting;

  USBH_TIME             IntLastErrorTime;
...
typedef struct {
  ...
  void      (*pfSetPortPower)                      (USBH_HUB_PORT * pPort, USBH_POWER_STATE State);
} USBH_EXT_HUB_API;
...
//lint -strong(AXJ, DEV_ENUM_STATE)
typedef enum {
  ...
  DEV_ENUM_CONFIGURE_EPS,        // Configure EPs (some drivers only)
  DEV_ENUM_INIT_HUB              // The device is an hub and is initialized
} DEV_ENUM_STATE;

//lint -strong(AXJ, USB_DEV_STATE)
...
struct _USB_DEVICE {
  ...
  U16                          MaxFifoSize;             // MaxPacketSize of EP 0
  ...
  unsigned                     CtrlTransferBufferSize;
...
**********************************************************************
*/
//lint -strong(AXJ, HOST_CONTROLLER_STATE)
typedef enum {
  HC_UNKNOWN,
  HC_REMOVED,
  HC_WORKING,
...
struct _USBH_HOST_CONTROLLER {                        // Global driver object
  ...
  USBH_DRIVER_CAPS              Caps;                 // Driver capabilities
  ...
  USBH_HC_HANDLE                pPrvData;             // Pointer to private driver data structure. It is passed to each function.
  U32                           UsbAddressUsed[4];    // Bit field to mark usage of up to 128 addresses.
  U8                            MaxAddress;           // 0 means, the driver allocates the address during SetAddress setup request
  U8                            Index;
  U8                            NextFreeAddress;
#if USBH_DELAY_BETWEEN_ENUMERATIONS > 0
  USBH_TIME                     NextPossibleEnum;     // Time stamp of the next possible enumeration process.
#endif
  ROOT_HUB                      RootHub;              // Embedded root hub
  USBH_HC_EP_HANDLE             RootEndpoints[4];
  ...
  unsigned                      FirstQueueItem;
  unsigned                      NumQueueItems;
  USBH_URB_QUEUE_ENTRY          UrbQueue[USBH_URB_QUEUE_SIZE];
  USBH_TIMER                    QueueRetryTimer;
#endif
};
...
*/
typedef struct {
  USBH_HOST_CONTROLLER          * aHostController[USBH_MAX_NUM_HOST_CONTROLLERS];
  ...
  const USBH_EXT_HUB_API        * pExtHubApi;
  struct {
  ...
  USBH_V2P_FUNC           * pfV2P;
#endif
  USBH_ON_PORT_EVENT_FUNC * pfOnPortEvent;
#if USBH_DEBUG > 1
  U32 DevUniqueID;
  U32 URBUniqueID;
#endif
...
#define USBH_AUDIO_INST_MAGIC                   FOUR_CHAR_ULONG('A','U','D','I')
#define USBH_AUDIO_CHN_MAGIC                    FOUR_CHAR_ULONG('A','U','D','o')
#define USBH_MIDI_INST_MAGIC                    FOUR_CHAR_ULONG('M','I','D','I')
#define USBH_CP210X_INST_MAGIC                  FOUR_CHAR_ULONG('C','P','2','1')
#define USBH_VIDEO_INST_MAGIC                   FOUR_CHAR_ULONG('V','I','D','E')
#define USBH_VIDEO_STREAM_MAGIC                 FOUR_CHAR_ULONG('V','I','D','S')
...
#else
  #define USBH_HCIndex2Inst(x)   USBH_Global.aHostController[x]
#endif
...
#else
  #define     USBH_SubmitRequest(pHC, Ep, pUrb)  (pHC)->pDriver->pfSubmitRequest(Ep, pUrb)
  #define     USBH_AbortEndpoint(pHC, Ep)        (pHC)->pDriver->pfAbortEndpoint(Ep)
#endif

USBH_STATUS   USBH_HCM_AllocContiguousMemory(U32 NumBytes, U32 Alignment, void ** ppVirtAddr, PTR_ADDR * pPhyAddr);

USBH_STATUS   USBH__AddNotification(USBH_NOTIFICATION_HOOK * pHook, USBH_NOTIFICATION_FUNC * pfNotification, void * pContext, USBH_NOTIFICATION_HOOK ** ppFirst, USBH_NOTIFICATION_HANDLE Handle);
...
#if defined(USBH_REF_DEBUG) && USBH_REF_DEBUG != 0 && USBH_SUPPORT_LOG != 0
  #define USBH_REF_TRACE 1
#else
  #define USBH_REF_TRACE 0
#endif
