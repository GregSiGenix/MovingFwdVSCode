...
#define MAX_NUM_SECTORS_BUFFER            0xFFFFu     // Maximum number of sectors that can be stored in a sector buffer.

/*********************************************************************
*
*       Defines related to Master Boot Record
*/
#define MBR_SECTOR_INDEX                  0u          // Index of the logical sector that stores the MBR.
#define MBR_PROTECTIVE_INDEX              0u          // Index of the partition table entry that stores the protective MBR.
...
#define PART_ENTRY_SIZE_SHIFT             4u
#define PART_ENTRY_OFF_START_SECTOR       0x08u
#define PART_ENTRY_OFF_NUM_SECTORS        0x0Cu

/*********************************************************************
*
*       Defines related to GUID partitions
*/
#define GPT_OS_TYPE                       0xEEu
#define GPT_CRC_INIT                      0xFFFFFFFFuL
#define GPT_HEADER_MAIN_SECTOR            1u
#define GPT_HEADER_MIN_SIZE               92u
#define GPT_HEADER_REVISION               0x00010000uL
#define GPT_HEADER_SIGNATURE              0x5452415020494645uLL   // 8 bytes ("EFI PART")
#define GPT_HEADER_SIZE                   92
#define GPT_HEADER_OFF_SIGNATURE          0u                      // Signature, 8 bytes
#define GPT_HEADER_OFF_REVISION           8u                      // Revision, 4 bytes
#define GPT_HEADER_OFF_SIZE               12u                     // HeaderSize, 4 bytes
#define GPT_HEADER_OFF_CRC                16u                     // HeaderCRC32, 4 bytes
#define GPT_HEADER_OFF_RESERVED           20u                     // Reserved, 4 bytes
#define GPT_HEADER_OFF_MY_SECTOR          24u                     // MyLBA, 8 bytes
#define GPT_HEADER_OFF_BACKUP_SECTOR      32u                     // AlternateLBA, 8 bytes
#define GPT_HEADER_OFF_FIRST_FS_SECTOR    40u                     // FirstUsableLBA, 8 bytes
#define GPT_HEADER_OFF_LAST_FS_SECTOR     48u                     // LastUsableLBA, 8 bytes
#define GPT_HEADER_OFF_DISK_ID            56u                     // DiskGUID, 16 bytes
#define GPT_HEADER_OFF_FIRST_ENTRY_SECTOR 72u                     // PartitionEntryLBA, 8 bytes
#define GPT_HEADER_OFF_NUM_ENTRIES        80u                     // NumberOfPartitionEntries, 4 bytes
#define GPT_HEADER_OFF_SIZE_OF_ENTRY      84u                     // SizeOfPartitionEntry, 4 bytes
#define GPT_HEADER_OFF_CRC_ENTRIES        88u                     // PartitionEntryArrayCRC32, 4 bytes
#define GPT_ENTRY_SIZE_OF_PART_NAME       72u
#define GPT_ENTRY_SIZE_SHIFT              7u
#define GPT_ENTRY_OFF_PART_TYPE           0u                      // PartitionTypeGUID, 16 bytes
#define GPT_ENTRY_OFF_PART_ID             16u                     // UniquePartitionGUID, 16 bytes
#define GPT_ENTRY_OFF_START_SECTOR        32u                     // StartingLBA, 8 bytes
#define GPT_ENTRY_OFF_END_SECTOR          40u                     // EndingLBA, 8 bytes
#define GPT_ENTRY_OFF_ATTR                48u                     // Attributes, 8 bytes
#define GPT_ENTRY_OFF_NAME                56u                     // PartitionName, 72 bytes
...
*/
typedef void (FS_STORAGE_TEST_HOOK_SECTOR_READ_BEGIN)(const FS_DEVICE_TYPE * pDeviceType, U8 DeviceUnit, U32 * pSectorIndex, void * pData, U32 * pNumSectors);

/*********************************************************************
*
*       FS_STORAGE_TEST_HOOK_SECTOR_READ_END
*/
typedef void (FS_STORAGE_TEST_HOOK_SECTOR_READ_END)(const FS_DEVICE_TYPE * pDeviceType, U8 DeviceUnit, U32 SectorIndex, void * pData, U32 NumSectors, int * pResult);                           //lint !e621 Identifier clash N:107. Rationale: This symbol is used only in test builds.

/*********************************************************************
*
*       FS_STORAGE_TEST_HOOK_SECTOR_WRITE_BEGIN
*/
typedef void (FS_STORAGE_TEST_HOOK_SECTOR_WRITE_BEGIN)(const FS_DEVICE_TYPE * pDeviceType, U8 DeviceUnit, U32 * pSectorIndex, const void ** ppData, U32 * pNumSectors, U8 * pRepeatSame);

/*********************************************************************
*
*       FS_STORAGE_TEST_HOOK_SECTOR_WRITE_END
*/
typedef void (FS_STORAGE_TEST_HOOK_SECTOR_WRITE_END)(const FS_DEVICE_TYPE * pDeviceType, U8 DeviceUnit, U32 SectorIndex, const void * pData, U32 NumSectors, U8 RepeatSame, int * pResult);     //lint !e621 Identifier clash  N:107. Rationale: This symbol is used only in test builds.
...
*    with the members of FS_INFO::Info
*/
typedef struct {
  U32                   NumSectors;                     // Total number of sectors available for the file system: RSVD + FAT + ROOT + DATA
  U16                   BytesPerSector;                 // Number of bytes in a logical sector. Typical values are: 512, 1024, 2048, and 4096.
  ...
  U8                    SectorsPerCluster;              // Number of sectors in an allocation unit.
...
*/
typedef struct {
  U32                   NumSectors;                     // Total number of sectors available for the file system: RSVD + EFS allocation table + DATA
  U16                   BytesPerSector;                 // Number of bytes in a logical sector. Typical values are: 512, 1024, 2048, and 4096.
  U16                   ldBytesPerSector;               // Number of bytes in a logical sector as a power of 2 exponent. Typical values are: 9, 10, 11, and 12.
  U32                   FirstDataSector;                // Index of the first logical sector that stores file or directory data.
  U32                   BytesPerCluster;                // Number of bytes in a cluster.
  U32                   EFSTableSize;                   // Number of sectors in the allocation table.
  U8                    SectorsPerCluster;              // Number of sectors in an allocation unit.
  U32                   NumClusters;                    // Total number of clusters on the storage device.
  U32                   NumFreeClusters;                // Once known, we keep track of the number of free clusters
  U32                   NextFreeCluster;                // Once known, we keep track of the next free cluster
...
*
*  Additional information
*    The members of the structure Info have to be identical
*    with the first 3 members of the FS_FAT_INFO and FS_EFS_INFO
*    structures.
*/
typedef union {
  FS_FAT_INFO FATInfo;              // Information related to FAT file system.
  FS_EFS_INFO EFSInfo;              // Information related to EFS file system.
  struct {
    U32       NumSectors;           // Total number of sectors available for the file system.
    U16       BytesPerSector;       // Number of bytes in a logical sector. Typical values are: 512, 1024, 2048, and 4096.
    U16       ldBytesPerSector;     // Number of bytes in a logical sector as a power of 2 exponent. Typical values are: 9, 10, 11, and 12.
  } Info;
} FS_INFO;
...
typedef struct {
  ...
  FS_VOLUME * pVolume;                // Volume on which the logical sector is stored.
...
struct FS_FILE {                                    //lint -esym(9058, FS_FILE) tag unused outside of typedefs N:999. Rationale: the type definition is used as forward declaration.
  ...
  FS_FILE                   * pNext;                // Next file handle in the list.
};
...
*
*  Description
*    Instance of a sector (or smart) buffer.
*/
struct FS_SB {                    //lint -esym(9058, FS_SB) tag unused outside of typedefs N:999. Rationale: the type definition is used as forward declaration.
  ...
  U32         OffSectorCopy;      // Offset of the sector relative to SectorIndex that has to be duplicated on write. Valid values are different than 0.
#endif // FS_MAINTAIN_FAT_COPY
#if FS_SUPPORT_SECTOR_BUFFER_BURST
  U32         SectorIndexRange;   // Minimum sector index that can be accessed.
  U32         NumSectorsRange;    // Maximum number of sectors that can be accessed. Set to 0 for an unlimited number of sectors.
  U16         NumSectorsBuffer;   // Number of sectors that can be stored in the buffer.
  U16         NumSectors;         // Number of sectors currently stored in the buffer.
  U16         OffSector;          // Offset of the sector relative to SectorIndex that is currently accessed by the file system.
#endif // FS_SUPPORT_SECTOR_BUFFER_BURST
  FS_VOLUME * pVolume;            // Instance of the volume on which the sector is stored.
  U8        * pBuffer;            // Sector data.
  U8          IsDirty;            // Set to 1 if the sector data has been modified and it has to be written to storage.
  I8          Error;              // Error code indicating the failure reason.
  U8          Type;               // Type of data stored to sector.
  U8          IsValid;            // Set to 1 if data is valid, usually because the sector has been read.
  U8          WriteToJournal;     // Set to 1 if the sector data hast to be written to journal.
#if FS_SUPPORT_SECTOR_BUFFER_BURST
  U8          IsExtBuffer;        // Set to 1 if the data buffer is provided by the application.
#endif // FS_SUPPORT_SECTOR_BUFFER_BURST
};
...
typedef struct {
  ...
  FS_DATA_BUFFER        * paDirEntryBuffer;
...
*/
void FS__DISKPART_SetTestHookSectorReadBegin (FS_STORAGE_TEST_HOOK_SECTOR_READ_BEGIN  * pfTestHook); //lint -esym(621, FS__DISKPART_SetTestHookSectorReadBegin) Identifier clash N:107. Rationale: This symbol is used only in test builds.
void FS__DISKPART_SetTestHookSectorReadEnd   (FS_STORAGE_TEST_HOOK_SECTOR_READ_END    * pfTestHook); //lint -esym(621, FS__DISKPART_SetTestHookSectorReadEnd) Identifier clash N:107. Rationale: This symbol is used only in test builds.
void FS__DISKPART_SetTestHookSectorWriteBegin(FS_STORAGE_TEST_HOOK_SECTOR_WRITE_BEGIN * pfTestHook); //lint -esym(621, FS__DISKPART_SetTestHookSectorWriteBegin) Identifier clash N:107. Rationale: This symbol is used only in test builds.
void FS__DISKPART_SetTestHookSectorWriteEnd  (FS_STORAGE_TEST_HOOK_SECTOR_WRITE_END   * pfTestHook); //lint -esym(621, FS__DISKPART_SetTestHookSectorWriteEnd) Identifier clash N:107. Rationale: This symbol is used only in test builds.
